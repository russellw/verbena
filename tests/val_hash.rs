...
fn test_val_float_in_hashset() {
    ...
    // Test Float values
    set.insert(Val::Num(3.14));
    set.insert(Val::Num(2.71));

    assert!(set.contains(&Val::Num(3.14)));
    assert!(set.contains(&Val::Num(2.71)));
    assert!(!set.contains(&Val::Num(1.23)));
    assert_eq!(set.len(), 2);

    // Special float values
    set.insert(Val::Num(f64::NAN));
    set.insert(Val::Num(f64::INFINITY));
    set.insert(Val::Num(f64::NEG_INFINITY));
    ...
    assert!(set.contains(&Val::Num(f64::INFINITY)));
    assert!(set.contains(&Val::Num(f64::NEG_INFINITY)));
}
...
fn test_val_in_hashmap() {
    ...
    assert_eq!(map.get(&Val::Num(3.14)), Some(&"pi"));
...
fn test_hashmap_with_val_values() {
    ...
    map.insert(Val::Str("key2".to_string()), Val::Num(3.14));
